...
- **Dig Research Integration**: Server-side HTTP proxy to the Innovatr Dig ETL API (`https://innovatr-dig-etl.vercel.app/api/dig/*`) at `/api/member/dig/*` (9 routes). Auth via `DIG_ETL_SERVICE_TOKEN` secret with `Authorization: Service <token>` and `X-Company-Id` headers. Typed HTTP client at `server/integrations/dig-etl.ts`. Client types in `client/src/lib/dig-api.types.ts` match ETL API response shapes. Hooks in `client/src/lib/dig-api.ts`. UI components in `client/src/components/dig/`. Search posts to `/api/member/dig/search` (proxied to ETL's vector search).
